...
- Wired CSV export “Share” snackbar action through the injectable share service so tapping Share opens the native sheet reliably, and added widget coverage for the interaction (`lib/core/di/injection_container.dart`, `lib/core/di/injection_container.config.dart`, `lib/presentation/pages/export/export_page.dart`, `test/widget/pages/export/export_page_test.dart`).
- Completed Doctor PDF download pipeline by persisting generated bytes via the shared `FileWriterService`, updated chart rendering capture to avoid hangs, and refreshed unit tests for the new binary/PNG flows (`lib/data/datasources/external/file_writer_service.dart`, `lib/data/datasources/external/pdf_generator_service.dart`, `lib/data/datasources/external/chart_rendering_service.dart`, `test/unit/data/datasources/external/*`).
...
- `ebeb2dd` feat: wire export page navigation — registers `/export` route, adds navigation entry from the home timeline, and introduces argument object for routing (`lib/presentation/router/*`, `lib/presentation/pages/home/reports_list_page.dart`, `lib/presentation/pages/export/export_page_args.dart`).
...
- `61091a5` fix: register export services in DI — wires CsvExportService/FileWriterService into GetIt and adds export navigation coverage (`lib/data/datasources/external/*`, `lib/core/di/injection_container.config.dart`, `test/widget/navigation/export_navigation_test.dart`).
